...
import { getUserResumes, saveUserResume, getResumeHistory, saveResumeVersion } from "@/lib/firestore";
import { UserResume, UserProfile, Template, ResumeHistory } from "@/types";
import { ArrowLeft, Loader2, Save, PenTool, Layout, Download, Trash2, Plus, RefreshCw, Sparkles, FileCode, ArchiveRestore, Target, CheckCircle2, AlertCircle, Info, History, X, FileText, Copy, FileIcon } from "lucide-react";
...
export const TEMPLATE_MAP: Record<string, React.FC<{ data: UserProfile }>> = {
...
export default function ResumeStudio() {
        ...
        if (resumeId) {
            ...
            if (targetResume) {
                ...
                if (selectedResume?.id !== targetResume.id) {
                    ...
                    setEditorData(targetResume.data ? JSON.parse(JSON.stringify(targetResume.data)) : null);
                    ...
                    // Fetch history
                    if (user?.uid) {
                        getResumeHistory(user.uid, targetResume.id).then(setHistory);
                    }
                }
            }
        }
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            await saveResumeVersion(user.uid, updatedResume.id, {
                data: editorData,
            ...
            // Refresh history
            const newHistory = await getResumeHistory(user.uid, updatedResume.id);
            ...
            fetch('/api/resume/thumbnail', {
                method: 'POST',
                body: JSON.stringify({ userId: user.uid, resumeId: updatedResume.id })
            }).catch(err => console.error("Thumbnail trigger error", err));
        } catch (error) {
    ...
    const handleRestoreVersion = (version: ResumeHistory) => {
        if (!window.confirm(`Restore this version from ${new Date(version.timestamp).toLocaleString()}? This will lose any unsaved changes.`)) return;

        setEditorData(JSON.parse(JSON.stringify(version.data)));
        if (version.templateId) setSelectedTemplateId(version.templateId);
    ...
    const handleEvaluateAts = async () => {
        ...
        try {
            ...
            // TRIGGER THUMBNAIL GENERATION ASYNC (to update score in thumbnail)
            fetch('/api/resume/thumbnail', {
                method: 'POST',
                body: JSON.stringify({ userId: user!.uid, resumeId: updatedResume.id })
            }).catch(err => console.error("Thumbnail trigger error", err));
    ...
    const handleGenerateCoverLetter = async () => {
        ...
        const jdToUse = selectedResume.jobDescription;
        if (!jdToUse) {
            toast.error("Please add a Job Description to the ATS panel first.");
            setShowAtsModal(true);
            return;
        }
        ...
        <div className="fixed inset-0 z-50 bg-black flex flex-col animate-fade-in">
            ...
            <div className="flex items-center justify-between px-6 py-3 border-b border-white/10 bg-black/90 backdrop-blur-md z-20">
                <div className="flex items-center gap-2 sm:gap-4">
                    <button
                        onClick={() => router.push('/profile?tab=resumes')}
                        className="text-gray-400 hover:text-white transition-colors flex items-center gap-1 sm:gap-2 text-[10px] sm:text-sm"
                    >
                        <ArrowLeft className="w-3 h-3 sm:w-4 h-4" /> <span className="hidden xs:inline">Library</span>
                    </button>
                    ...
                    <input
                        ...
                        className="bg-transparent text-sm sm:text-lg font-bold text-white focus:outline-none focus:border-b border-purple-500 w-24 sm:w-64 overflow-hidden text-ellipsis"
                    />
                </div>
                <div className="flex items-center gap-1.5 sm:gap-3">
                    <div className="hidden xl:block">
                        <select
                            ...
                            className="bg-transparent border border-white/20 text-gray-300 text-[10px] rounded-lg px-2 py-1 outline-none focus:border-purple-500 w-32 text-ellipsis hover:bg-white/5 transition-colors cursor-pointer"
                        >
                            <option value="" disabled className="bg-[#1a1a1a]">AI Engine</option>
                    ...
                    <div className="hidden lg:flex items-center gap-2">
                        <button
                            ...
                            className="flex items-center gap-1.5 px-3 py-1.5 bg-purple-500/10 text-purple-400 rounded-lg font-bold text-[10px] border border-purple-500/20 hover:bg-purple-500/20 transition-all"
                        >
                            <FileText className="w-3.5 h-3.5" /> Letter
                        </button>
                        <button
                            onClick={() => setShowHistory(true)}
                            className="flex items-center gap-1.5 px-3 py-1.5 bg-white/5 text-gray-300 rounded-lg font-bold text-[10px] border border-white/10 hover:bg-white/10 transition-all"
                        >
                            <History className="w-3.5 h-3.5" /> History
                        </button>
                        <button
                            onClick={handleRestoreOriginal}
                            className="flex items-center gap-1.5 px-3 py-1.5 bg-red-500/10 text-red-400 rounded-lg font-bold text-[10px] border border-red-500/20 hover:bg-red-500/20 transition-all"
                        >
                            <ArchiveRestore className="w-3.5 h-3.5" /> Original AI
                        </button>
                    </div>
                    ...
                    <div className="flex items-center gap-1.5 sm:gap-2">
                        <button
                            ...
                            className="flex items-center gap-2 px-3 py-1.5 sm:py-2 bg-blue-600/10 hover:bg-blue-600/20 text-blue-400 rounded-lg font-bold border border-blue-500/20 transition-all text-[10px] sm:text-xs"
                        >
                            {downloadingDocx ? <Loader2 className="w-3 h-3 animate-spin" /> : <FileIcon className="w-3 h-3" />}
                            <span className="hidden sm:inline">Word</span>
                        </button>
                        <button
                            onClick={() => reactToPrintFn()}
                            className="flex items-center gap-2 px-3 py-1.5 sm:py-2 bg-white/5 hover:bg-white/10 text-gray-300 rounded-lg font-bold border border-white/10 transition-all text-[10px] sm:text-xs"
                        >
                    ...
                    <button
                        onClick={handleSave}
                        disabled={saving}
                        className="flex items-center gap-2 px-3 sm:px-5 py-1.5 sm:py-2 bg-purple-600 hover:bg-purple-500 text-white rounded-lg font-bold shadow-lg shadow-purple-900/20 disabled:opacity-50 transition-all text-[10px] sm:text-xs"
                    >
            ...
            <div className="flex-1 flex flex-col md:flex-row overflow-hidden relative">
                ...
                <div className="md:hidden flex bg-[#0f0f12] border-t border-white/10 p-2 gap-2 z-30">
                    <button 
                        onClick={() => setActiveTab("edit")}
                        className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-bold transition-all ${activeTab === 'edit' ? 'bg-purple-600 text-white' : 'bg-white/5 text-gray-400'}`}
                    >
                        <PenTool className="w-4 h-4" /> Edit
                    </button>
                    <button 
                        onClick={() => setActiveTab("preview")}
                        className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-bold transition-all ${activeTab === 'preview' ? 'bg-purple-600 text-white' : 'bg-white/5 text-gray-400'}`}
                    >
                ...
                {/* LEFT: EDITING PANEL */}
                <div className={`${activeTab === 'edit' ? 'flex' : 'hidden'} md:flex w-full md:w-[400px] lg:w-[480px] bg-[#0A0A0A] border-r border-white/10 overflow-y-auto p-4 sm:p-6 space-y-8 pb-32 flex-col`}>
                    
                    {/* Quick Model Selector for Mobile */}
                    <div className="md:hidden space-y-2">
                        <p className="text-[10px] uppercase font-bold text-gray-500 tracking-widest">AI Engine</p>
                        <select
                            onChange={(e) => setSelectedEnhanceModel(e.target.value)}
                            value={selectedEnhanceModel}
                            className="w-full bg-white/5 border border-white/10 text-gray-300 text-xs rounded-lg px-3 py-2 outline-none focus:border-purple-500"
                        >
                    ...
                    {/* Toolbar Actions (Rest of buttons relocated for mobile sanity) */}
                    <div className="flex flex-wrap gap-2 md:hidden">
                        <button
                            onClick={handleGenerateCoverLetter}
                            disabled={generatingCoverLetter}
                            className="flex items-center gap-2 px-3 py-2 bg-purple-500/10 text-purple-400 rounded-lg font-bold text-xs border border-purple-500/20"
                        >
                            <FileText className="w-4 h-4" /> Cover Letter
                        </button>
                        <button
                            onClick={handleDownloadDocx}
                            className="flex items-center gap-2 px-3 py-2 bg-blue-500/10 text-blue-400 rounded-lg font-bold text-xs border border-blue-500/20"
                        >
                            <FileIcon className="w-4 h-4" /> DOCX
                        </button>
                        <button
                            onClick={() => reactToPrintFn()}
                            className="flex items-center gap-2 px-3 py-2 bg-white/5 text-gray-300 rounded-lg font-bold text-xs border border-white/10"
                        >
                        ...
                        <button
                            onClick={handleRestoreOriginal}
                            className="flex items-center gap-2 px-3 py-2 bg-red-500/10 text-red-400 rounded-lg font-bold text-xs border border-red-500/20"
                        >
                            <ArchiveRestore className="w-4 h-4" /> Original AI
                        </button>
                    </div>
                    ...
                    <div className="space-y-4">
                        <div className="flex justify-between items-center">
                            <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider flex items-center gap-2" onClick={() => setShowTemplateMenu(!showTemplateMenu)}>
                                <Layout className="w-4 h-4" /> Template Style 
                                <span className="md:hidden text-[10px] bg-white/5 px-2 py-0.5 rounded ml-2">{showTemplateMenu ? 'Hide' : 'Show'}</span>
                            </h3>
                        </div>
                        <div className={`${showTemplateMenu ? 'block' : 'hidden'} md:block`}>
                            <TemplateSelector
                                templates={templates}
                ...
                {/* RIGHT: LIVE PREVIEW */}
                <div className={`${activeTab === 'preview' ? 'flex' : 'hidden'} md:flex flex-1 bg-[#1a1a1a] p-4 sm:p-8 lg:p-12 overflow-y-auto justify-center`}>
                    <div className="w-full max-w-[210mm] transition-transform origin-top scale-[0.45] xs:scale-[0.55] sm:scale-[0.7] md:scale-[0.8] lg:scale-[0.9] xl:scale-100">
                                    ...
                                    {pdfPreviewUrl ? (
                                        ...
                                        <>
                                            ...
                                            <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-4">
                                                ...
                                                <p className="text-gray-300 text-xs sm:text-sm max-w-md text-center px-4">
                                                ...
                                                <button
                                                    ...
                                                    disabled={compiling}
                                                    className="px-6 py-3 bg-red-600 hover:bg-red-700 text-white rounded-xl font-bold shadow-lg transition-transform hover:scale-105 flex items-center gap-2 disabled:opacity-50 disabled:scale-100"
                                                >
            ...
            {/* Resume History Timeline Overlay */}
            {showHistory && (
                <div className="absolute top-[64px] right-0 bottom-0 w-80 bg-[#0f0f12] border-l border-white/10 z-40 animate-fade-in-left flex flex-col">
                    ...
                    <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
                        {history.length === 0 ? (
                            ...
                            history.map((version, i) => (
                                ...
                                >
                                    <div className="absolute left-[-5px] top-0 w-2.5 h-2.5 rounded-full bg-purple-500 ring-4 ring-[#0f0f12]" />
                                    <div className="bg-white/5 border border-white/10 rounded-xl p-3 hover:border-purple-500/50 transition-all group">
                                        <div className="flex justify-between items-start mb-1">
                                            <p className="text-[10px] font-bold text-purple-400 uppercase tracking-tighter">
                                                {i === 0 ? "Latest Version" : `Version ${history.length - i}`}
                                            </p>
                                            <p className="text-[10px] text-gray-500">
                                                {new Date(version.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                            </p>
                                        </div>
                                        <p className="text-xs text-white font-medium mb-1">{version.label || "Manual Save"}</p>
                                        <p className="text-[10px] text-gray-500 mb-3">{new Date(version.timestamp).toLocaleDateString()}</p>
                                        <button 
                                            onClick={() => handleRestoreVersion(version)}
                                            className="w-full py-1.5 rounded-lg bg-white/5 hover:bg-white/10 text-[10px] font-bold text-gray-300 hover:text-white transition-all border border-white/5"
                                        >
                    ...
                    <div className="p-4 bg-white/5 border-t border-white/10">
                        <p className="text-[10px] text-gray-500 leading-relaxed italic">
                            A new snapshot is created automatically every time you click "Save Changes".
                        </p>
                    </div>
                </div>
            )}
            ...
            {showCoverLetterModal && (
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm animate-fade-in">
                    <div className="glass-panel border-white/10 rounded-2xl w-full max-w-3xl overflow-hidden flex flex-col shadow-2xl h-[80vh]">
                        <div className="p-6 border-b border-white/10 flex justify-between items-center bg-white/5">
                            ...
                            <button onClick={() => setShowCoverLetterModal(false)} className="text-gray-400 hover:text-white">
                                <X className="w-5 h-5" />
                            </button>
                        </div>
                        
                        <div className="flex-1 overflow-y-auto p-8 bg-white/[0.02] custom-scrollbar">
                            <div className="max-w-2xl mx-auto bg-white p-12 text-gray-800 shadow-2xl rounded-sm min-h-full font-serif leading-relaxed text-sm whitespace-pre-wrap">
                                {coverLetter}
                            </div>
                        </div>
                        ...
                        <div className="p-6 border-t border-white/10 bg-black flex gap-3">
                            <button 
                                ...
                                className="flex-1 px-4 py-3 rounded-xl border border-white/10 text-white font-bold hover:bg-white/10 transition-all flex items-center justify-center gap-2"
                            >
                            ...
                            <button 
                                onClick={() => {
                                    ...
                                    const a = document.createElement('a');
                                    a.href = url;
                                    a.download = `CoverLetter_${resumeTitle.replace(/\s+/g, '_')}.txt`;
                                    a.click();
                                    toast.success("Downloading as TXT...");
                                }}
                                className="flex-1 px-6 py-3 bg-purple-600 hover:bg-purple-500 text-white rounded-xl font-bold shadow-lg shadow-purple-900/20 transition-all flex items-center justify-center gap-2"
                            >
